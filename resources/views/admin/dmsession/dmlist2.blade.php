...
@section('container')
                                ...
                                <tbody>
                                    @foreach ($studsession as $data)
                                        ...
                                        <tr>
                                            ...
                                            <td style="text-align: center;">
                                                @if ($data->status == '0')
                                                    <a href="{{ route('admin.dmsessioncancel', $data->id) }}" type="button"
                                                        class="btn" style="background: #95949E;color:#fff;">Alloted</a>
                                                @elseif ($data->status == '1')
                                                    <a href="{{ route('admin.dmsessioncancel', $data->id) }}" type="button"
                                                        class="btn" style="background: #007BFF;color:#fff;">Alloted</a>
                                            ...
                                            <td style="text-align: center;">
                                                <button type="button" class="btn" data-bs-toggle="modal"
                                                    onclick="getTakeAwayData('{{ $data->student_name }}','{{ $data->expert_name }}','{{ $data->date }}','{{ $data->time }}','{{ $data->mode }}','{{ $data->venue }}','{{ $data->takeaway }}','{{ $data->admintakeaway }}','{{ $data->stud_file }}')"
                                                    data-bs-target="#takeawayModal"
        ...
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="info-label">Takeaway:</div>
    ...
    {{--  <!-- Cancel Session Model -->
    <div class="modal fade bd-example-modal-s" id="sessionCancelModal" tabindex="-1" role="dialog"
        aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-s" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Cancel Session of Student</h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('admin.dmsessioncancel') }}" method="post">
                    @csrf
                    <div class="modal-body text-center">
                        <div class="row">
                            <input type="hidden" name="session_id" id="session_id">
                            <div class="col-lg-12">
                                <div class="info-label"> Cancel Session:</div>
                                <input type="radio" name="status" id="cancel" value="cancel">
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
    ...
    <script>
        ...
        function getTakeAwayData(student_name, expert_name, date, time, mode, venue, takeaway, admintakeaway, stud_file) {
            document.getElementById('stud_name').innerHTML = student_name;
            document.getElementById('expert_name').innerHTML = expert_name;
            document.getElementById('date').innerHTML = date;
            document.getElementById('time').innerHTML = time;
            document.getElementById('mode').innerHTML = mode;
            document.getElementById('venue').innerHTML = venue;
            document.getElementById('admintakeaway').innerHTML = admintakeaway;
            //console.log(venue);
            //document.getElementById('takeaway').innerHTML = takeaway;
            let sessionFileContainer = document.getElementById('sessin_file_container');
            let taskAnswers = document.getElementById('takeaway');
            if (takeaway == undefined) {
                console.log('data not found');
            } else {
                if (takeaway) {
                    sessionFileContainer.innerHTML = '';
                    taskAnswers.innerHTML = '';
                    let answers = takeaway;
                    document.getElementById('takeaway').innerHTML = takeaway;
                } else {
                    sessionFileContainer.innerHTML = '';
                    taskAnswers.innerHTML = '';
                    let sessionFile = stud_file;
                    let filePath = `/takeaway_file/${sessionFile}`;

                    if (sessionFile.endsWith('.jpg') || sessionFile.endsWith('.png')) {
                        let img = document.createElement('img');
                        img.src = filePath;
                        img.alt = "Task Image";
                        img.style.maxWidth = "100%";
                        sessionFileContainer.appendChild(img);
                    } else if (sessionFile.endsWith('.pdf')) {
                        let iframe = document.createElement('iframe');
                        iframe.src = filePath;
                        iframe.width = "100%";
                        iframe.height = "500px";
                        sessionFileContainer.appendChild(iframe);
                    } else if (sessionFile.endsWith('.doc') || sessionFile.endsWith('.docx') || sessionFile.endsWith(
                            '.xls') ||
                        sessionFile
                        .endsWith('.xlsx')) {
                        let fileMessage = document.createElement('p');
                        fileMessage.innerHTML =
                            `File: <a href="${filePath}" target="_blank">${sessionFile.substring(sessionFile.lastIndexOf('/') + 1)}</a>`;
                        sessionFileContainer.appendChild(fileMessage);
                    } else {
                        let fileMessage = document.createElement('p');
                        fileMessage.innerText = `File: ${sessionFile.substring(sessionFile.lastIndexOf('/') + 1)}`;
                        sessionFileContainer.appendChild(fileMessage);
                    }
                }
            }
