...
@section('container')
                                    ...
                                    <div class="mb-3">
                                        <div class="row">
                                            ...
                                            <div class="col-2 text-end">
                                                <button type="button" id="addLeaderName" class="btn btn-primary "
                                                    style="margin-top: -10px;"><i class='fas fa-plus'
                                                        style='font-size:12px'></i></button>
                                            </div>
                                            <div class="col-12">
                                                <div id="leaderNamesContainer" style="">
                                            ...
                                            <td>
                                                @foreach ($leaderNames as $leader_name)
                                                    @foreach ($student as $stud)
                                                        @if ($stud->name == $leader_name)
                                                            <button type="button" class="btn mt-1" data-bs-toggle="modal"
                                                                data-bs-target="#staticBackdrop"
                                                                onclick="getCCData('{{ $data->id }}', '{{ $stud->id }}')"
                                                                style="background: lightgray;">View</button>
                                                            <br>
                                                        @endif
                                                    @endforeach
                                                @endforeach
    ...
    <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Minute of Meeting(MOM)</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="info-data" id="mom"></div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#school_id').change(function() {
                ...
                $('#leader_name').val(null).trigger('change');
                $('#leader_name option').each(function() {
                    var schoolId = $(this).data('schoolid');
                    if (schoolId == selectedSchoolId || selectedSchoolId === '') {
                        $(this).show();
                    } else {
                        $(this).hide();
                    }
        ...
        $(document).ready(function() {
            ...
            // Function to filter students based on selected school
            function filterStudentsBySchool(schoolId) {
                $('#leader_name').empty(); // Clear current options
                $('#leader_name').append('<option value="">Select Multiple Students</option>'); // Add placeholder

                // Loop through all students and add only those that match the selected school
                @foreach ($student as $studlist)
                    if (schoolId == '{{ $studlist->school_id }}') {
                        $('#leader_name').append(
                            '<option value="{{ $studlist->name }}" data-schoolid="{{ $studlist->school_id }}">{{ $studlist->name }}</option>'
                        );
                    }
                @endforeach

                $('#leader_name').select2(); // Reinitialize Select2
            }

            // On school change, filter the students
            $('#school_id').change(function() {
                var selectedSchoolId = $(this).val();
                filterStudentsBySchool(selectedSchoolId);
            });

            // Trigger change to filter students on page load if a school is already selected
            $('#school_id').trigger('change');
        });

        function getCCData(id, studid) {
            // Filter all MOMs related to the given id and studid
            let momList = momData.filter(p => p.cc_id == id && p.studId == studid);

            // Sort MOMs in descending order by created_at date
            momList.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));

            let momContainer = document.getElementById('mom');
            momContainer.innerHTML = ''; // Clear previous content

            if (momList.length > 0) {
                momList.forEach(mom => {
                    let momItem = document.createElement('div');
                    momItem.className = 'mom-item'; // You can style this class in your CSS
                    momItem.innerHTML = `
                <div class="mom-header">
                    <p class="mom-date">${new Date(mom.created_at).toLocaleString()}</p>
                </div>
                <p>${mom.mom}</p>
                <hr>
            `;
                    momContainer.appendChild(momItem);
                });
            } else {
                momContainer.innerHTML = '<p>No MOM data available.</p>';
            }
