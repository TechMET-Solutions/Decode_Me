...
@section('content')
    <style>
        ...
        summary::after {
            ...
            background-image: url('{{ asset('user/assets/images/star1.png') }}');
    ...
    <div class="app-wrapper">
        <div class="app-content pt-3 p-md-3 p-lg-4">
            <div class="container-xl">
                {{-- <h1 class="app-page-title">My Career Option Details</h1> --}}
            </div><!--//container-fluid-->
        </div><!--//app-content-->
    </div>
    <div class="app-wrapper">
                    ...
                    <div class="col-lg-8 col-sm-12 mb-4">
                        ...
                        <div class="d-flex justify-content-center align-items-center mt-2">
                            ...
                            <a type="button" class="btn" href="{{ route('careerClub') }}"
                                style="border-radius: 20px;margin-left:30px; font-size:20px;font-weight:500;background: #FFFAF0;border: 1px #8D5D00 solid">&nbsp;&nbsp;&nbsp;&nbsp;Back
                                &nbsp;&nbsp;&nbsp;&nbsp;</a>
                        </div>
                        <div class="row">
                            <div class="col-lg-12 col-sm-12">
                                <div class="d-flex justify-content-between mt-4"
                                    style="box-shadow: -4px 0px 0px #FFED7E; border-radius: 12px; padding: 20px;background:#fff;">
                                    <div class="d-flex align-items-center">
                                        <div>
                                            <p class="card-text" style="font-size: 24px; margin: 0;">{{ $career->name }}</p>
                                            <p>{!! $career->desc !!}</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-lg-12 col-sm-12 mt-4">
                                <div class="d-flex justify-content-between">
                                    <p class="task-title" style="color: #8D5D00;">Career Paths</p>
                                    <p class="task-name" style="color: #71BF0D">Top Priority</p>
                                </div>
                                <section>
                                    @php
                                    ...
                                    @if (!empty($rank))
                                        @foreach ($rank as $key => $value)
                                            @foreach ($subCareer as $data)
                                                @if ($data->id == $key)
                                                    <details>
                                                        <summary data-image-number="{{ $value }}">
                                                            {{ $data->name }}
                                                        </summary>
                                                        <div>{!! $data->desc !!}</div>
                                                    </details>
                                    ...
                                    @foreach ($subCareer as $data2)
                                        @if (!array_key_exists($data2->id, $rank))
                                            <details>
                                                <summary data-image-number="{{ '-' }}">
                                                    {{ $data2->name }}
                                                </summary>
                                                <div>{!! $data2->desc !!}</div>
                                            </details>
                                    ...
                                    {{--  @foreach ($subCareer as $data2)
                                     @foreach ($rank as $key2 => $value2)
                                      @if ($data2->id != $key2)
                                          <details>
                                                        <summary data-image-number="{{ '0' }}">
                                                            {{ $data->name }}
                                                        </summary>
                                                        <div>{!! $data->desc !!}</div>
                                                    </details>
                                      ...
                                      @endforeach
                                    @endforeach  --}}
                                    {{--  @foreach ($subCareer as $index => $data)
                                        @if ($rank != '')
                                            @foreach ($rank as $key => $value)
                                                @if ($data->id == $key)
                                                    <details>
